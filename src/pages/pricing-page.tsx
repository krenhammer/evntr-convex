...
import { useSession } from "@/auth/use-auth-hooks"
...
function PricingPage() {
  ...
  // Redirect authenticated users to assistants page
  useEffect(() => {
    if (status !== 'loading' && session?.user) {
      router.navigate({ to: '/assistants' })
    }
  ...
  // Don't render the pricing page for authenticated users
  if (session?.user) {
    return null
  }
  ...
  const handleSubscription = async (plan: PlanType) => {
    try {
      setLoading(plan)
      // Redirect to assistants page after selecting a plan
      // Note: You'll need to implement actual subscription handling with Clerk
      // This is a placeholder implementation
      router.navigate({ 
        to: '/assistants',
        // You can pass plan information in search params if needed
        search: { plan }
      })
    } catch (error) {
